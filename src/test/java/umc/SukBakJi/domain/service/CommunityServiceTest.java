...
class CommunityServiceTest {
    ...
    public void 최신질문글조회_정상작동() {
        ...
        assertEquals("박사 질문글", latestQuestions.get(0).getTitle());

        assertEquals(Menu.석사, latestQuestions.get(1).getMenu());
        assertEquals("석사 질문글", latestQuestions.get(1).getTitle());

        assertEquals(Menu.진학예정, latestQuestions.get(2).getMenu());
        assertEquals("진학예정 질문글", latestQuestions.get(2).getTitle());
    }
}